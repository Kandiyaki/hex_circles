package net.hex_circles.casting.patterns.spells
...
import net.hex_circles.networking.SetLookPitchS2CMsg
...
class OpCongrats : SpellAction {
    ...
    override fun execute(args: List<Iota>, ctx: CastingContext): Triple<RenderedSpell, Int, List<ParticleSpray>> {
        ...
        if (target !is ServerPlayerEntity) throw MishapBadEntity(
            target,
            Text.translatable("text.hex_circles.congrats.player")
        )
    ...
    private data class Spell(val player: ServerPlayerEntity) : RenderedSpell {
        override fun cast(ctx: CastingContext) {
            player.sendMessage(Text.translatable("text.hex_circles.congrats", player.displayName));
            Hex_circlesNetworking.sendToPlayer(player, SetLookPitchS2CMsg(-90f))
        }
    }
}
